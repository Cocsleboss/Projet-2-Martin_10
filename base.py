...
def GraphPL():
    ...
    if request.method =="POST":
        ...
        if famille == "None":
            ...
            res = res.fetchall()
            nombre = len(res)
            return render_template("pleine-lune.html", vache_naissance = [345, 478], mois = mois, annee = annee, velage = nombre)
        ...
        if famille != "None":
            ...
            res = res.fetchall()
            nombre = len(res)
            return render_template("pleine-lune.html", vache_naissance = [50, 50], mois = mois, annee = annee, nombre_velage = nombre)
            #return f"voila {res}"
